...
from engine.routers.auth import verify_session, authenticate, AuthenticationFields
...
def test_authenticate_valid(generate_user):
    db = TestingSessionLocal()
    auth_fields = AuthenticationFields(
        username="test",
        password="test"
    )
...
def test_authenticate_invalid(generate_user):
    db = TestingSessionLocal()
    auth_fields = AuthenticationFields(
        username="test",
        password="tet"
...
def test_sign_in_invalid(generate_user):
    response = client.post(
        "/api/auth/v1/sign-in", 
        json={"username": "test", "password":"tst"})
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
    

def test_get_artist_no_session():
    response = client.get("/api/web/v1/artist?youtube_channel_id=UC0Q7Hlz75NYhYAuq6O0fqHw")
